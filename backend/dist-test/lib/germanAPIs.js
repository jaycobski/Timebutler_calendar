...
exports.getGermanApiClient = exports.GermanApiClient = exports.ApiError = exports.ApiErrorType = exports.CircuitBreakerState = void 0;
...
(function (ApiErrorType) {
    ...
    ApiErrorType["CIRCUIT_BREAKER_OPEN"] = "circuit_breaker_open";
})(ApiErrorType || (exports.ApiErrorType = ApiErrorType = {}));
class ApiError extends Error {
    constructor(message, type, provider, statusCode, originalError) {
        super(message);
        this.name = 'ApiError';
        this.type = type;
    ...
    async execute(operation) {
        if (this.state === CircuitBreakerState.OPEN) {
            if (Date.now() - this.lastFailureTime < this.config.recoveryTimeout) {
                throw new ApiError(`Circuit breaker is OPEN for ${this.providerName}`, ApiErrorType.CIRCUIT_BREAKER_OPEN, this.providerName);
            }
 ...
 */
const transformFeiertageApi = (data, stateCode, year) => {
    const holidays = [];
 ...
 */
const transformBundesregierungApi = (data, stateCode, year) => {
 ...
 */
const transformBundApi = (data, stateCode, year) => {
    return data.feiertage.map(feiertag => ({
        ...
        date: feiertag.datum,
        type: feiertag.kategorie === 'bundesweit' ? 'federal' : 'state',
...
// =====================================================
const createApiProviders = () => {
    const env = (0, env_1.getEnvConfig)();
    return [
        {
            name: 'feiertage-api',
...
class GermanApiClient {
    ...
    async fetchFromProvider(provider, stateCode, year) {
    ...
    validateInputs(stateCode, year) {
        if (stateCode !== 'ALL' && !/^[A-Z]{2}$/.test(stateCode)) {
            throw new ApiError(`Invalid state code: ${stateCode}`, ApiErrorType.VALIDATION_ERROR, 'client');
        }
        if (year < 2025 || year > 2026) {
            throw new ApiError(`Year must be within range 2025-2026, got: ${year}`, ApiErrorType.VALIDATION_ERROR, 'client');
        }
    }
    validateHolidayData(holidays, stateCode, year) {
    ...
    categorizeError(error, providerName) {
        ...
        if (axios_1.default.isAxiosError(error)) {
            ...
            if (error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT') {
                return new ApiError(`Request timeout for ${providerName}`, ApiErrorType.TIMEOUT_ERROR, providerName, status, error);
            }
            if (status === 429) {
                return new ApiError(`Rate limit exceeded for ${providerName}`, ApiErrorType.RATE_LIMIT_ERROR, providerName, status, error);
            }
            if (status === 401 || status === 403) {
                return new ApiError(`Authentication failed for ${providerName}`, ApiErrorType.AUTHENTICATION_ERROR, providerName, status, error);
            }
            if (status === 404) {
                return new ApiError(`Resource not found for ${providerName}`, ApiErrorType.NOT_FOUND_ERROR, providerName, status, error);
            }
            if (status && status >= 500) {
                return new ApiError(`Server error for ${providerName}`, ApiErrorType.SERVER_ERROR, providerName, status, error);
            }
            return new ApiError(`Network error for ${providerName}: ${error.message}`, ApiErrorType.NETWORK_ERROR, providerName, status, error);
        }
        return new ApiError(`Unknown error for ${providerName}: ${error.message || 'Unknown error'}`, ApiErrorType.NETWORK_ERROR, providerName, undefined, error);
    }
    updateSuccessMetrics(providerName, responseTime) {
    ...
    updateFailureMetrics(providerName, error) {
        const metrics = this.metrics.get(providerName);
        metrics.totalRequests++;
    ...
    generateFallbackData(stateCode, year) {
        ...
        const easterDate = luxon_1.DateTime.fromISO(easter);
        holidays.push({
            id: `karfreitag-${year}`,
            name_de: 'Karfreitag',
            name_en: 'Good Friday',
        ...
        return holidays.filter(h => stateCode === 'ALL' ||
            h.states.includes('ALL') ||
            h.states.includes(stateCode));
    }
    getStateSpecificFallbackHolidays(stateCode, year, easterDate) {
        const holidays = [];
        // Catholic states
...
exports.GermanApiClient = GermanApiClient;
// =====================================================
// Utility Functions
// =====================================================
function mapHolidayNameToId(name, year) {
    const mapping = {
...
function getHolidayNames(germanName) {
    const translations = {
...
function getEnglishTranslation(germanName) {
    return getHolidayNames(germanName).name_en;
}
function determineHolidayType(holidayName) {
...
function determineHolidayStates(holidayName, currentState) {
    const federalHolidays = [
        'Neujahr', 'Karfreitag', 'Ostermontag', 'Tag der Arbeit',
        'Christi Himmelfahrt', 'Pfingstmontag', 'Tag der Deutschen Einheit',
        '1. Weihnachtstag', '2. Weihnachtstag'
...
// Singleton Export
// =====================================================
let germanApiClientInstance = null;
const getGermanApiClient = () => {
    if (!germanApiClientInstance) {
        germanApiClientInstance = new GermanApiClient();
    }
    return germanApiClientInstance;
};
exports.getGermanApiClient = getGermanApiClient;
