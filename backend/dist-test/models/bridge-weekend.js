...
const VALID_BRIDGE_PATTERNS = ['thursday-friday', 'monday-tuesday', 'tuesday-friday', 'sandwich', 'extend-weekend'];
// Performance cache for bridge calculations (<100ms requirement)
const bridgeCalculationCache = new Map();
...
class BridgeWeekend {
     ...
     */
    validateBridgeWeekendData(data) {
     ...
     */
    overlapsWithSchoolHolidays(stateCode) {
        // Simplified check for Bavaria - would need full school holiday data for all states
        if (stateCode === 'BY') {
            const bridgeStart = luxon_1.DateTime.fromISO(this.start_date);
            const bridgeEnd = luxon_1.DateTime.fromISO(this.end_date);
            return BAVARIAN_SCHOOL_HOLIDAYS_2025.some(schoolHoliday => {
                const schoolStart = luxon_1.DateTime.fromISO(schoolHoliday.start);
                const schoolEnd = luxon_1.DateTime.fromISO(schoolHoliday.end);
     ...
     */
    calculateGermanVacationROI(params) {
     ...
     */
    static createFromHoliday(holiday, // Holiday-like object
    stateCode, pattern) {
        if (!GERMAN_STATES.includes(stateCode)) {
            throw new Error(`Invalid German state code: ${stateCode}`);
        }
        const holidayDate = luxon_1.DateTime.fromISO(holiday.date, { zone: 'Europe/Berlin' });
     ...
     */
    static createMegaBridge(holidays, stateCode) {
        ...
        const firstDate = luxon_1.DateTime.fromISO(firstHoliday.date, { zone: 'Europe/Berlin' });
        const lastDate = luxon_1.DateTime.fromISO(lastHoliday.date, { zone: 'Europe/Berlin' });
        ...
        while (currentDate <= endDate) {
            ...
            const isHoliday = sortedHolidays.some(h => luxon_1.DateTime.fromISO(h.date).hasSame(currentDate, 'day'));
     ...
     */
    static createFromOverlappingHolidays(holidays, stateCode) {
        ...
        const firstDate = luxon_1.DateTime.fromISO(sortedHolidays[0].date);
        const secondDate = luxon_1.DateTime.fromISO(sortedHolidays[1].date);
        ...
        return new BridgeWeekend({
            id: bridgeId,
            holiday_id: sortedHolidays.map((h) => h.id).join(','),
            state_code: stateCode,
            start_date: startDate.toFormat('yyyy-MM-dd'),
            end_date: endDate.toFormat('yyyy-MM-dd'),
            vacation_days_needed: vacationDaysNeeded,
     ...
     */
    static optimizeMayCluster(mayHolidays, stateCode, maxVacationDays) {
        const bridges = [];
        mayHolidays.forEach(holiday => {
            const holidayDate = luxon_1.DateTime.fromISO(holiday.date);
     ...
     */
    static getBestBridgesForState(stateCode, year, maxVacationDays) {
        if (!GERMAN_STATES.includes(stateCode)) {
            throw new Error(`Invalid German state code: ${stateCode}`);
        }
     ...
     */
    static findOptimalBridges(stateCode, year, constraints) {
        ...
        if (constraints.preferMayBridges) {
            ...
            bridges = bridges.sort((a, b) => {
                ...
                if (aMay !== bMay)
                    return bMay - aMay; // May bridges first
     ...
     */
    static optimizeForGermanWorkCulture(stateCode, year, preferences) {
        const constraints = {
     ...
     */
    static async calculateAllBridges(stateCode, year) {
     ...
     */
    static sortByEfficiency(bridges) {
     ...
     */
    static getApplicablePatternsForWeekday(weekday) {
        const patterns = [];
        switch (weekday) {
            case 1:
                patterns.push('monday-tuesday');
     ...
     */
    static getMockHolidaysForState(stateCode, year) {
